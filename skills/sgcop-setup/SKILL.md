...
3. 選んだ参照先で `.rubocop.yml` の `inherit_gem` を整える。何を書いたかを一言報告する:
   ...
   ```
4. `inherit_gem` と同時に `inherit_mode: merge: [Exclude]` も設定する。`.rubocop.yml` 編集時に
   ...
   同じ Cop に持つ `Exclude` を配列ごと丸ごと上書きする（[rubocop#9983](https://github.com/rubocop/rubocop/issues/9983)、
...
- **方針確認を最初に**: 標準か strict かで、後段（重複削除の対象範囲、カスタム Cop の扱い、ノイズ量）が
  すべて変わる。手順 3 を飛ばさない。
- **`inherit_mode` を先に設定する**: `inherit_gem`（手順 3）を整えたら、`inherit_mode: merge:
  [Exclude]` も同時に設定する。これが無いと、`.rubocop_todo.yml`（手順 6）や手順 7 の `Exclude`

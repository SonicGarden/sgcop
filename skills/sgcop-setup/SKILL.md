...
- 参照先の候補:
  ...
  - 純 Ruby → 標準 `ruby/rubocop.yml` / strict `ruby/rubocop_strict.yml`
...
2. **AskUserQuestion** で「strict 一括 / 標準」を選ばせる。選んだ参照先:
   ...
   - 純 Ruby → strict `ruby/rubocop_strict.yml` / 標準 `ruby/rubocop.yml`
3. 選んだ参照先で `.rubocop.yml` の `inherit_gem` を整える。何を書いたかを一言報告する:
   ...
   ```yaml
   inherit_gem:
     sgcop: rails/rubocop_strict.yml   # 標準なら rails/rubocop.yml、純 Ruby なら ruby/...
   ```
...
1. **sgcop 側の設定を動的取得する**。インストール済み sgcop gem の実体パスを取得し、選んだ設定
   ...
   - strict を選んだ場合 → `rails/rubocop_strict.yml`（→ `rubocop.yml` → `../ruby/rubocop.yml`、
     さらに `../ruby/rubocop_strict.yml`）や、純 Ruby なら `ruby/rubocop_strict.yml` の継承チェーンを辿る。
   - 標準を選んだ場合 → `rails/rubocop.yml`（→ `../ruby/rubocop.yml`）等。

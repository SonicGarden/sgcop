...
- **Tier 2: `rubocop -A`（unsafe autocorrect）** — safe で残った分に当てる。**挙動が変わりうる**ので、
  ...
  ただし手順 6 の関連テスト検証は省かない（無確認で当てる分、テストが唯一の安全網）。テストが失敗
...
- **サイクルの最後に、必ずプロジェクト全体で `bundle exec rubocop`（`--only` 無し、対象ファイル
  ...
  - **それでも全体実行で違反が残っていたら、このサイクルは未完了として扱い、原因を調査してから
    ...
    - autocorrect の副作用でスコープ外の Cop が新規違反を出した → その違反を Tier 1〜3
      の要領でこのサイクル内に含めて直す（経路Bの「1 回 1 Cop」原則は、あくまで
      **選定時に複数 Cop を狙って着手しない**という意味であり、副作用の後始末まで対象を
      広げるかどうかを縛るものではない）。
    - 経路Aでバッチから切り離した Cop のエントリを todo に戻し忘れている → 手順 4 の
...
- **対象 Cop に絞って実行する。** `--only <Cop>` を付けないと、まだ todo に残した他 Cop の違反まで
  出力に混ざって、潰したい Cop の進捗が見えにくくなる。ただし手順 6 最後の全体検証だけは
  `--only` を付けずに実行する（この検証の目的そのものが「絞り込みでは見えない副作用が無いか」の
  確認なので、絞ってしまうと意味がない）。
